...
class CapsuleViewSet(ModelViewSet):
    ...
    def preview(self, request, pk):
        ...
        return Response({"message": "Cannot access this capsule"}, status=status.HTTP_401_UNAUTHORIZED)
    ...
    def retrieve(self, request, pk):
        ...
        if request.user != capsule.member and not capsule.is_open:
            return Response({"message": "Cannot access this capsule"}, status=status.HTTP_401_UNAUTHORIZED)
        return Response(
            {"data": serializer.data, "message": "Here's a glimpse of your stored memories."}, status=status.HTTP_200_OK
        )
    ...
    def join_capsule(self, request, pk):
        ...
        if serializer.is_valid:
            ...
            return Response(serializer.data, status=status.HTTP_200_OK)
        return Response({"message": "Capsule or user not found"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def add_new_member_to_capsule(self, request, capsule_id, user_id):
        ...
        if request.user == capsule.creator and request.user not in capsule.contributors:
            ...
            return capsule
        return Response({"message": "Unauthorized to share this capsule"}, status=status.HTTP_401_UNAUTHORIZED)
